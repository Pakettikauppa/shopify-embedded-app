...
class DbSessionStorage implements SessionStorage
{
    ...
    {
        $dbSession = \App\Models\Shopify\Session::where('session_id', $sessionId)->first();
    ...
    {
        $dbSession = \App\Models\Shopify\Session::where('session_id', $session->getId())->first();
        if (!$dbSession) {
            $dbSession = new \App\Models\Shopify\Session();
        }
    ...
    {
        return \App\Models\Shopify\Session::where('session_id', $sessionId)->delete() === 1;
    }
}